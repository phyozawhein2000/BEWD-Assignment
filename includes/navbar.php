...
<div id="joinModal" class="fixed inset-0 z-[150] hidden flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl transform transition-all scale-95 opacity-0" id="modalContent">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-emerald-800 tracking-tight">Join FoodFusion</h2>
        ...
        <p class="text-stone-600 mb-8 leading-relaxed">Become a member to share recipes, discover culinary secrets, and connect with food lovers worldwide!</p>
        <div class="space-y-3">
            <a href="auth/register.php" class="block w-full text-center bg-emerald-800 text-white py-4 rounded-2xl font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-900/20 transition-all active:scale-[0.98]">Create Account</a>
            <a href="auth/login.php" class="block w-full text-center border border-stone-200 py-4 rounded-2xl font-bold text-stone-700 hover:bg-stone-50 transition-all">Login</a>
        </div>
    </div>
</div>
...
<div class="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-stone-100 z-[100] px-4 pb-6 pt-3 flex justify-around items-center shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
    ...
    <a href="recipes.php" class="flex flex-col items-center text-stone-400 hover:text-emerald-800">
    ...
    <button onclick="toggleMobileSidebar()" class="relative -mt-12">
    ...
    <a href="community.php" class="flex flex-col items-center text-stone-400">
    ...
    <a href="<?php echo isset($_SESSION['user_id']) ? 'profile.php' : 'auth/login.php'; ?>" class="flex flex-col items-center text-stone-400">
        ...
        <div class="mt-auto pt-8 border-t border-stone-100">
            <?php if(isset($_SESSION['user_id'])): ?>
                <div class="flex items-center space-x-4 mb-6">
                    <div class="h-12 w-12 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-800 font-bold text-xl">
                        <?php echo substr($_SESSION['user_name'], 0, 1); ?>
                    </div>
                    <div>
                        <p class="text-xs text-stone-400 font-bold uppercase">Account</p>
                        <p class="font-bold text-stone-800"><?php echo $_SESSION['user_name']; ?></p>
                    </div>
                </div>
                <a href="auth/logout.php" class="block w-full text-center py-4 rounded-2xl bg-red-50 text-red-600 font-bold">Logout</a>
            <?php else: ?>
                <div class="grid grid-cols-1 gap-3">
                    <a href="auth/register.php" class="bg-emerald-800 text-white text-center py-4 rounded-2xl font-bold shadow-lg shadow-emerald-900/20">Sign Up Free</a>
                    <a href="auth/login.php" class="text-stone-500 text-center py-3 font-bold">Sign In</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
